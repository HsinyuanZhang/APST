...
from apst.dandi_subm.carrier import estimate_move_t4, fit_carrier_normalizer
...
def require_full_source(records: list[SessionData]) -> None:
    if len(records) != len(protocol.TRAIN_SESSIONS) or {r.session_id for r in records} != set(protocol.TRAIN_SESSIONS):
        raise ValueError("formal fitting requires all and only frozen sub-M source sessions")
