...
def resolve_data_root(raw_root: str | Path | None = None) -> Path:
    """Resolve raw NWB storage from an explicit path or ``APST_DATA_ROOT``.

    The explicit argument takes precedence.  This package never derives a data
    location from a checkout or a machine-specific workspace path.
    """
    root = raw_root if raw_root is not None else os.environ.get("APST_DATA_ROOT")
    if root is None or not str(root).strip():
        raise ValueError(
            "provide raw_root explicitly or set APST_DATA_ROOT to the NWB directory"
        )
...
def _canonical_metadata(
    nwb: Any, units_df: Any
) -> tuple[list[tuple[str, str, int, str]], np.ndarray, np.ndarray, np.ndarray]:
    electrodes = nwb.electrodes.to_dataframe()
    required = {"group_name", "location", "bank", "pin", "label"}
    if not required <= set(electrodes.columns):
        raise ValueError(
            f"electrode table lacks required columns: {sorted(required - set(electrodes.columns))}"
        )
    key_by_id: dict[int, tuple[str, str, int, str]] = {}
    for electrode_id, row in electrodes.iterrows():
        if (
            str(row.group_name) == "electrode_group_M1"
            and str(row.location) == "Primary Motor Cortex"
        ):
            key_by_id[int(electrode_id)] = (
                str(row.group_name),
                str(row.bank),
                int(row.pin),
                str(row.label),
    ...
    if len(canonical) != protocol.CANONICAL_ELECTRODES or len(set(canonical)) != len(
        canonical
    ):
    ...
    return (
        ...
        unit_map,
        observed,
        np.asarray(selected_unit_indices, dtype=np.int64),
    )
...
def _resample_trial(values: np.ndarray) -> np.ndarray:
    ...
    return interp1d(x_old, values, axis=0, kind=kind, bounds_error=True)(x_new).astype(
        np.float32
    )
...
def load_pair(
    ...
    *,
    ...
    purpose: Literal["source", "development", "final"],
) -> dict[str, SessionData]:
    """Open one authorized NWB once and return identical-timebase SUA and PMUA views."""
    split = protocol.split_for(session_id)
    if split != purpose:
        raise PermissionError(f"{session_id} is {split}, not {purpose}")
    raw_root = resolve_data_root(raw_root)
    ...
    with NWBHDF5IO(str(nwb_path), "r", load_namespaces=True) as io:
        ...
        canonical_keys, unit_map, observed, selected_unit_indices = _canonical_metadata(
            nwb, units
        )
        if not (0 < len(selected_unit_indices) <= protocol.MAX_UNITS):
            raise ValueError(
                f"{session_id}: M1 unit count {len(selected_unit_indices)} is outside 1..{protocol.MAX_UNITS}"
            )
        spikes = [
            np.asarray(units.iloc[index]["spike_times"], dtype=np.float64)
            for index in selected_unit_indices
        ]
        all_spikes = np.concatenate(spikes)
        edges = np.arange(
            float(all_spikes.min()),
            float(all_spikes.max()) + protocol.BIN_SECONDS,
            protocol.BIN_SECONDS,
        )
        neural_sua = np.stack(
            [np.histogram(s, bins=edges)[0] for s in spikes], axis=1
        ).astype(np.float32)
        velocity_series = nwb.processing["behavior"]["Velocity"].time_series[
            "cursor_vel"
        ]
        vel_times, vel_raw = np.asarray(velocity_series.timestamps[:]), np.asarray(
            velocity_series.data[:]
        )
        centers = (edges[:-1] + edges[1:]) / 2.0
        velocity = np.stack(
            [
                np.interp(centers, vel_times, vel_raw[:, axis], left=0.0, right=0.0)
                for axis in range(vel_raw.shape[1])
            ],
            axis=1,
        ).astype(np.float32)
    ...
    query_indices = np.asarray(
        [
            start + protocol.WINDOW_BINS - 1
            for trial in trials[protocol.Q50_TRIAL_INDEX :]
            for start in range(trial["start"], trial["stop"] - protocol.WINDOW_BINS + 1)
        ],
        dtype=np.int64,
    )
    support_indices = np.concatenate(
        [
            np.arange(t["start"], t["stop"], dtype=np.int64)
    ...
    for trial in trials[: protocol.CARRIER_TRIALS]:
        if trial["go_cue_time"] is None:
            raise ValueError(
                f"{session_id}: M33 carrier trial lacks finite go_cue_time"
            )
        left = trial["go_cue_time"] + protocol.MOVE_START_AFTER_GO_SECONDS
        right = trial["go_cue_time"] + protocol.MOVE_STOP_AFTER_GO_SECONDS
        if not (
            trial["start_time"] <= left < right <= trial["stop_time"]
            and edges[0] <= left < right <= edges[-1]
        ):
            raise ValueError(
                f"{session_id}: MOVE window is not contained in trial/raw support"
            )
        # Exact physical-time, half-open counts; no arbitrary 20 ms-grid rounding.
        carrier_counts_sua.append(
            np.asarray(
                [
                    np.searchsorted(s, right, side="left")
                    - np.searchsorted(s, left, side="left")
                    for s in spikes
                ],
                dtype=np.float32,
            )
    ...
    carrier_angles = np.asarray(
        [
            t["target_dir"] if t["target_dir"] is not None else np.nan
            for t in trials[: protocol.CARRIER_TRIALS]
        ],
        dtype=np.float64,
    )
    carrier_angle_audit = validate_carrier_angles(carrier_angles)
    activity_sua = np.stack(
        [
            _resample_trial(neural_sua[t["start"] : t["stop"]])
            for t in trials[: protocol.ACTIVITY_TRIALS]
        ]
    )
    ...
    activity_pmua = np.zeros(
        (protocol.ACTIVITY_TRIALS, 100, pmua_channels.size), dtype=np.float32
    )
    np.add.at(
        activity_pmua.transpose(2, 0, 1), inverse, activity_sua.transpose(2, 0, 1)
    )
    activity_pmua_direct = np.stack(
        [
            _resample_trial(neural_pmua[t["start"] : t["stop"]])
            for t in trials[: protocol.ACTIVITY_TRIALS]
        ]
    )
    ...
    carrier_counts_pmua = np.zeros(
        (protocol.CARRIER_TRIALS, pmua_channels.size), dtype=np.float32
    )
    ...
    base_metadata = {
        "schema": "dandi688_bench_v2_session_v1",
        "protocol": protocol.protocol_dict(),
        "raw_nwb_sha256": _file_sha256(nwb_path),
        "raw_trial_rows": [t["raw_trial_row"] for t in trials],
        ...
        **carrier_angle_audit,
        ...
        "window_receipts": [
            {
                "raw_trial_row": t["raw_trial_row"],
                "go_cue_time": t["go_cue_time"],
                "move_start": t["go_cue_time"] + 0.1,
                "move_stop": t["go_cue_time"] + 0.6,
            }
            for t in trials[: protocol.CARRIER_TRIALS]
        ],
    }
    ...
    def make(
        ...
        return SessionData(
            ...
            carrier_indices,
            activity,
            arrays["carrier_counts"],
            arrays["carrier_angles"].astype(np.float64),
            channels,
            unit_map,
            observed,
            metadata,
    ...
    return {
        "sua": make(
            "sua", neural_sua, activity_sua, unit_map.copy(), carrier_counts_sua_array
        ),
        "pmua": make(
            "pmua", neural_pmua, activity_pmua, pmua_channels, carrier_counts_pmua
        ),
    }


def padded_windows(
    record: SessionData, indices: np.ndarray, n_pad: int = protocol.MAX_UNITS
    ...
    if (
        indices.ndim != 1
        or np.any(indices < protocol.WINDOW_BINS - 1)
    ...
    unitmask = np.zeros(n_pad, dtype=bool)
    unitmask[: record.neural.shape[1]] = True
...
def save_session(data: SessionData, path: Path) -> None:
    ...
    try:
        np.savez_compressed(
            temp, **payload, metadata=np.asarray(json.dumps(metadata, sort_keys=True))
        )
...
def load_cached_session(path: Path) -> SessionData:
    with np.load(Path(path), allow_pickle=False) as archive:
        ...
        if split not in {"source", "development", "final"}:
        ...
        if (
            metadata.get("query_index_semantics")
            != "inclusive end bin of a 50-bin causal window; target is velocity[query_indices]"
        ):
    ...
    if (
        representation not in {"sua", "pmua"}
        or neural.ndim != 2
        or not (0 < neural.shape[1] <= protocol.MAX_UNITS)
    ):
        raise ValueError("cache representation/neural geometry mismatch")
    if (
        values["velocity"].shape != (neural.shape[0], 2)
        or values["bin_edges"].shape != (neural.shape[0] + 1,)
    ...
    if (
        values["query_indices"].ndim != 1
        or not len(values["query_indices"])
    ...
    if (
        not np.all(np.isfinite(values["neural"]))
        or not np.all(np.isfinite(values["velocity"]))
        or not np.all(np.isfinite(values["activity"]))
        or not np.all(np.isfinite(values["carrier_counts"]))
    ):
        raise ValueError("cache non-angle arrays must be finite")
    carrier_audit = validate_carrier_angles(values["carrier_angles"])
    if (
        metadata.get("finite_direction_count")
        != carrier_audit["finite_direction_count"]
        or metadata.get("direction_design_rank")
        != carrier_audit["direction_design_rank"]
    ):
        raise ValueError("cache carrier direction audit mismatch")
    selected = metadata.get("selected_original_unit_indices")
    if (
        not isinstance(selected, list)
        or values["unit_to_electrode"].shape != (len(selected),)
        or np.any(values["unit_to_electrode"] < 0)
        or np.any(values["unit_to_electrode"] >= protocol.CANONICAL_ELECTRODES)
    ):
        raise ValueError("cache unit-to-electrode provenance mismatch")
    if (
        values["support_indices"].ndim != 1
        or not len(values["support_indices"])
        or np.any(values["support_indices"] < 0)
        or np.any(values["support_indices"] >= neural.shape[0])
        or not set(values["carrier_indices"].tolist())
        <= set(values["support_indices"].tolist())
        or set(values["carrier_indices"].tolist())
        & set(values["query_indices"].tolist())
    ):
