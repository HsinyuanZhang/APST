...
from .frontend import whole_unit_dropout
from ._identity import BTransformerUnifiedDecoderIdentity
...
def _geometry(
    task: str | Mapping[str, Any], context_bins: int
) -> tuple[str, dict[str, int]]:
    if isinstance(task, str):
        if task not in TASK_GEOMETRIES:
            raise ValueError(
                f"unknown RIFT task {task!r}; use h1, m1, m2, or an explicit geometry"
            )
    ...
    geometry = {
        ...
        if key in (*required, "window", "prefix")
    }
...
class RiftDecoder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        default_context = (
            TASK_GEOMETRIES[task]["window"]
            if isinstance(task, str) and task in TASK_GEOMETRIES
            else 300
        )
        ...
        if not isinstance(context_bins, int) or context_bins < 5:
            raise ValueError(
                "context_bins must be an integer >= frontend kernel size 5"
            )
    ...
    def _check_input(self, x: Tensor, input_valid_mask: Tensor | None) -> Tensor:
        if x.ndim != 3 or x.shape[0] < 1 or x.shape[1] < 1 or x.shape[2] != self.units:
            raise ValueError(
                f"x must be nonempty [B,T,{self.units}], got {tuple(x.shape)}"
            )
        if not torch.isfinite(x).all():
            raise ValueError("x contains non-finite values")
        if input_valid_mask is not None:
            if (
                input_valid_mask.dtype != torch.bool
                or input_valid_mask.shape != x.shape[:2]
            ):
    ...
    def frontend_tokens(
        self,
        x: Tensor,
        bank: TaskBank | Sequence[TaskBank],
        ...
        keep = self._resolve_keep(
            banks, unit_mask, x.shape[0], x.device, dropout_generator, dropout_keep
        )
        e0 = torch.stack([torch.from_numpy(value.E0) for value in banks]).to(
            x.device, dtype=torch.float32
        )
        carrier = torch.stack([torch.from_numpy(value.carrier) for value in banks]).to(
            x.device, dtype=torch.float32
        )
        return self._batched_proj_add_frontend(x, e0, carrier, keep)

    def _batched_proj_add_frontend(
        self, x: Tensor, e0: Tensor, carrier: Tensor, keep: Tensor
    ) -> Tensor:
    ...
    def _fuse_batched_local(
        self, local: Tensor, e0: Tensor, carrier: Tensor, keep: Tensor
    ) -> Tensor:
        ...
        fused_local = (
            local.unsqueeze(-2) + proj.view(batch, 1, units, groups, 16)
        ).reshape(batch, width, units, -1)
        tokens = self.frontend.token_norm(
            self.frontend.token_mlp(
                torch.cat(
                    [
                        fused_local,
                        carrier.to(dtype=local.dtype)
                        .unsqueeze(1)
                        .expand(batch, width, units, -1),
                    ],
                    dim=-1,
                )
            )
        ...
        slots = (
            self.frontend.slot_norm(self.frontend.slots)
            .view(1, 1, 8, 256)
            .expand(batch, width, 8, 256)
        )
        ...
        pad = (
            (~keep)
            .unsqueeze(1)
            .expand(batch, width, units)
            .reshape(batch * width, units)
        )
        attended, _ = self.frontend.mha(
            q, k, k, key_padding_mask=pad, need_weights=False
        )
        slots_out = q + attended
        slots_out = slots_out + self.frontend.slot_ffn(
            self.frontend.slot_ffn_norm(slots_out)
        )
    ...
    def frontend_last(
        ...
        e0 = torch.stack([torch.from_numpy(value.E0) for value in banks]).to(
            raw5.device, dtype=torch.float32
        )
        carrier = torch.stack([torch.from_numpy(value.carrier) for value in banks]).to(
            raw5.device, dtype=torch.float32
        )
        ...
        local = (
            conv.act(conv.conv(flat))
            .reshape(batch, self.units, 16, 1)
            .permute(0, 3, 1, 2)
        )
        return self._fuse_batched_local(local, e0, carrier, keep)[:, 0]

    def _normalise_banks(
        self, bank: TaskBank | Sequence[TaskBank], batch: int
    ) -> list[TaskBank]:
        banks = [bank] * batch if isinstance(bank, TaskBank) else list(bank)
        if len(banks) != batch or not all(
            isinstance(value, TaskBank) for value in banks
        ):
            raise ValueError(
                "bank must be a TaskBank or a sequence of one TaskBank per batch row"
            )
        for value in banks:
            if value.E0.shape != (
                self.units,
                self.geometry["e0_dim"],
            ) or value.carrier.shape != (self.units, 4):
    ...
    def _resolve_keep(
        ...
        if dropout_keep is not None:
            ...
            return (
                keep.expand(batch, -1).contiguous()
                if keep.shape[0] == 1
                else keep.contiguous()
            )
        ...
        if self.training and self._frontend_owner.unit_dropout_p > 0:
            keep = whole_unit_dropout(
                keep, self._frontend_owner.unit_dropout_p, dropout_generator
            )
    ...
    def forward(
        ...
        return self.forward_scores(
            x, bank, unit_mask, dropout_generator, dropout_keep, input_valid_mask
        )[:, -1]


__all__ = ["RiftDecoder", "TASK_GEOMETRIES"]

APSTDecoder = RiftDecoder

__all__ = ["APSTDecoder", "RiftDecoder", "TASK_GEOMETRIES"]
