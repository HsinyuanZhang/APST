...
def _normalise_rows(
    rows: Tensor | Sequence[int], batch_size: int, device: torch.device
) -> Tensor:
...
class RiftTemporal(nn.Module):
    ...
    def __init__(self, config: RiftTemporalConfig) -> None:
        ...
        slopes = [
            0.0 if half_life is None else log(2.0) * config.bin_seconds / half_life
            for half_life in config.half_life_seconds
        ]
        if config.bias_mode == "flat":
            slopes = [0.0] * config.heads
        self.register_buffer(
            "recency_slopes", torch.tensor(slopes, dtype=torch.float32), persistent=True
        )
    ...
    def select_rows(
        self, state: RiftTemporalState, row_indices: Tensor | Sequence[int]
    ) -> RiftTemporalState:
    ...
    def forward(
        self, z: Tensor, valid_mask: Tensor | None = None, *, backend: str | None = None
    ) -> Tensor:
        ...
        if valid_mask is not None:
            if valid_mask.shape != z.shape[:2] or valid_mask.dtype != torch.bool:
                raise ValueError(
                    "valid_mask must be a bool tensor with shape [batch, time]"
                )
        ...
        if valid_mask is None:
            ...
            raise ValueError(
                "valid_mask may only contain left padding, not internal or right holes"
            )
    ...
    def _local_attention(
        self, q: Tensor, k: Tensor, v: Tensor, valid_mask: Tensor, window: int
    ) -> Tensor:
    ...
    def step(
        ...
        for layer_index, (block, window) in enumerate(
            ...
            logits = logits - self.recency_slopes.to(dtype=z.dtype).view(
    ...
    def _validate_state(self, state: RiftTemporalState) -> None:
        ...
        if (
            len(state.keys) != self.config.layers
            or len(state.values) != self.config.layers
        ):
        ...
        for layer_index, window in enumerate(self.config.windows):
            if (
                len(state.keys[layer_index]) != state.batch_size
                or len(state.values[layer_index]) != state.batch_size
            ):
            ...
            for keys, values in zip(state.keys[layer_index], state.values[layer_index]):
                if len(keys) != len(values) or len(keys) > window - 1:
                    raise ValueError(
                        "state cache is malformed or exceeds configured capacity"
                    )
