...
from ..temporal import RiftTemporal, RiftTemporalState, _normalise_rows
...
class LearnableRecencyState(RiftTemporalState):
    ...
    def select_rows(
        self, row_indices: Tensor | Sequence[int]
        ...
        return LearnableRecencyState(
            keys=[
                [[entry for entry in layer[index]] for index in selected]
...
class LearnableRecencyTemporal(RiftTemporal):
        ...
        try:
            if steal_from is None:
                ...
                self.register_buffer(
                    "recency_slopes",
                    steal_from.recency_slopes.detach().clone(),
                    persistent=True,
                )
            ...
            if recency_cfg.half_life_seconds != self.config.half_life_seconds:
                if len(recency_cfg.half_life_seconds) != self.config.heads:
                    raise ValueError(
                        "learnable half_life_seconds must match temporal heads"
                    )
    ...
    def from_initialized(
        cls, temporal: RiftTemporal, recency_cfg: LearnableRecencyConfig
    ...
    def _install_learnable_parameters(self) -> None:
        ...
        active = slopes > 0
        learn_mask = (
            torch.ones(heads, dtype=torch.bool, device=device)
            if cfg.learn_flat_heads
            else active
        )
    ...
    def copy_exported_slopes_(self, dest: RiftTemporal) -> Tensor:
        exported = self.export_shared_slope_vector().to(
            device=dest.recency_slopes.device
        )
        dest.recency_slopes.copy_(exported)
        return exported

    def _increments_and_gates(
        self, tokens: Tensor, layer_index: int
    ) -> tuple[Tensor, Tensor]:
        ...
        if cfg.tier == "learned_slope":
            slopes = self.effective_slopes().to(
        ...
        if cfg.tier != "fixed":
            raise ValueError(f"unknown tier {cfg.tier!r}")
        increments = base.view(1, 1, heads).expand(batch, time, heads)
        return increments, tokens.new_ones(batch, time, heads)

    def _masked_increments(self, increments: Tensor, valid_mask: Tensor) -> Tensor:
        return torch.where(
            valid_mask.unsqueeze(-1), increments, torch.zeros_like(increments)
        )

    def _apply_bias(
        self, logits: Tensor, increment_sums: Tensor, gates: Tensor
    ) -> Tensor:
    ...
    def _forward_dense(self, z: Tensor, valid_mask: Tensor) -> Tensor:
        ...
        for layer_index, (block, window) in enumerate(
            zip(self.blocks, self.config.windows)
        ):
            ...
            qkv = block.qkv(block.norm1(h)).view(
                z.shape[0], time, 3, self.config.heads, self.config.head_dim
            )
            q, k, v = qkv.unbind(dim=2)
            logits = torch.einsum("bthd,bshd->bhts", q, k) * (
                self.config.head_dim**-0.5
            )
            base = self.recency_slopes.to(device=z.device, dtype=z.dtype).view(
                1, self.config.heads, 1, 1
            )
            da = increments - self.recency_slopes.to(
                device=z.device, dtype=z.dtype
            ).view(1, 1, self.config.heads)
            prefix = da.cumsum(dim=1)
            delta = prefix.permute(0, 2, 1).unsqueeze(-1) - prefix.permute(
                0, 2, 1
            ).unsqueeze(-2)
            increment_sums = (
                base * ages.to(dtype=z.dtype).view(1, 1, time, time) + delta
            )
            ...
            logits = torch.where(
                valid_mask[:, None, :, None], logits, torch.zeros_like(logits)
            )
            weights = torch.softmax(logits, dim=-1)
            attention = torch.einsum("bhts,bshd->bthd", weights, v).reshape(
                z.shape[0], time, self.config.width
            )
    ...
    def _local_attention(
        ...
        k_windows = functional.pad(k.permute(0, 2, 3, 1), (window - 1, 0)).unfold(
            -1, window, 1
        )
        v_windows = functional.pad(v.permute(0, 2, 3, 1), (window - 1, 0)).unfold(
            -1, window, 1
        )
        k_windows = k_windows.permute(0, 3, 1, 4, 2)
        v_windows = v_windows.permute(0, 3, 1, 4, 2)
        key_valid = functional.pad(valid_mask, (window - 1, 0), value=False).unfold(
            1, window, 1
        )
        ...
        if increments is None:
            ...
            a_windows = functional.pad(
                increments.permute(0, 2, 1), (window - 1, 0)
            ).unfold(-1, window, 1)
            ages = torch.arange(window - 1, -1, -1, device=q.device, dtype=q.dtype)
            increment_sums = self._age_plus_delta_sums(a_windows, ages)
            gate = (
                gates.permute(0, 2, 1).unsqueeze(-1)
        ...
        weights = torch.softmax(logits, dim=-1)
        return torch.einsum("bhtk,bthkd->bthd", weights, v_windows).reshape(
            batch, time, self.config.width
        )
    ...
    def _forward_local(self, z: Tensor, valid_mask: Tensor) -> Tensor:
        ...
        for layer_index, (block, window) in enumerate(
            ...
            q, k, v = qkv.unbind(dim=2)
            attention = self._local_attention(
                q, k, v, valid_mask, window, increments, gates
            )
            residual = h + block.out(attention)
            h = residual + block.ffn(block.norm2(residual))
            h = torch.where(valid_mask.unsqueeze(-1), h, torch.zeros_like(h))
    ...
    def step(
        ...
        if z.shape[-1] != self.config.width:
            raise ValueError(
                f"z final dimension must equal configured width {self.config.width}"
            )
        ...
        if self.recency_cfg.tier != "learned_slope" and not isinstance(
            state, LearnableRecencyState
        ):
            raise TypeError("step() requires LearnableRecencyState increment cache")
        ...
        for layer_index, (block, window) in enumerate(
            ...
            g_now = gates[:, 0]
            qkv = block.qkv(block.norm1(h)).view(
                state.batch_size, 3, self.config.heads, head_dim
            )
            q, k, v = qkv.unbind(dim=1)
            cache_capacity = window - 1
            keys = z.new_zeros(
                state.batch_size, cache_capacity + 1, self.config.heads, head_dim
            )
            values = z.new_zeros(
                state.batch_size, cache_capacity + 1, self.config.heads, head_dim
            )
            cached_a = z.new_zeros(
                state.batch_size, cache_capacity + 1, self.config.heads
            )
            key_valid = torch.zeros(
                state.batch_size, cache_capacity + 1, dtype=torch.bool, device=z.device
            )
            increment_lists = (
                state.increments if isinstance(state, LearnableRecencyState) else None
            )
            for row in range(state.batch_size):
                old_keys = state.keys[layer_index][row]
                old_values = state.values[layer_index][row]
                old_increments = (
                    increment_lists[layer_index][row]
                    if increment_lists is not None
                    else []
                )
                if old_keys:
                    old_count = len(old_keys)
                    keys[row, cache_capacity - old_count : cache_capacity] = (
                        torch.stack(old_keys)
                    )
                    values[row, cache_capacity - old_count : cache_capacity] = (
                        torch.stack(old_values)
                    )
                    key_valid[row, cache_capacity - old_count : cache_capacity] = True
                    if old_increments:
                        cached_a[row, cache_capacity - old_count : cache_capacity] = (
                            torch.stack(old_increments)
                        )
                if valid_rows[row]:
                ...
                if valid_rows[row] and window > 1:
                    ...
                    state.values[layer_index][row] = [*old_values, v[row]][
                        -(window - 1) :
                    ]
                    if increment_lists is not None:
                        increment_lists[layer_index][row] = [
            ...
            if self.recency_cfg.tier == "learned_slope" and increment_lists is None:
                ages = torch.arange(
                    cache_capacity, -1, -1, device=z.device, dtype=z.dtype
                )
                slopes = self.effective_slopes().to(dtype=z.dtype)
                if slopes.ndim == 2:
                    slopes = slopes[layer_index]
                logits = logits - slopes.view(1, self.config.heads, 1) * ages.view(
                    1, 1, -1
                )
            else:
                ages = torch.arange(
                ...
                increment_sums = self._age_plus_delta_sums(
                    cached_a.permute(0, 2, 1), ages
                )
            ...
            weights = torch.softmax(logits, dim=-1)
            attention = torch.einsum("bhk,bkhd->bhd", weights, values).reshape(
                state.batch_size, self.config.width
            )
            residual = h + block.out(attention)
            h = residual + block.ffn(block.norm2(residual))
            tokens = h.unsqueeze(1)
        state.positions = [
            position + int(is_valid)
            for position, is_valid in zip(state.positions, valid_rows)
        ]
    ...
    def _validate_state(self, state: RiftTemporalState) -> None:
        ...
        for layer_index, window in enumerate(self.config.windows):
            ...
            for keys, increments in zip(
                state.keys[layer_index], state.increments[layer_index]
            ):
                if self.recency_cfg.tier != "learned_slope" and (
                    len(increments) != len(keys) or len(increments) > window - 1
                ):
                    raise ValueError(
                        "state increment cache is malformed or exceeds configured capacity"
                    )

    @torch.no_grad()
    def increment_statistics(
        self, z: Tensor, valid_mask: Tensor | None = None
    ) -> dict[str, object]:
        ...
        for layer_index, (block, _window) in enumerate(
            zip(self.blocks, self.config.windows)
        ):
            ...
            attention = self._local_attention(
                ...
                v,
                valid_mask,
                self.config.windows[layer_index],
                increments,
                _gates,
            )
            residual = h + block.out(attention)
        ...
        quantiles = torch.quantile(
            values, torch.tensor([0.10, 0.50, 0.90], device=values.device), dim=0
        )
        return {
