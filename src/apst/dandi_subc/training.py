...
from apst.legacy_dandi.carrier import apply_carrier_normalizer, estimate_move_t4
from .common import (PairedSampler, aggregate_scores, atomic_json, digest,
...
def _hash_source() -> dict[str, str]:
    """Bind the portable release implementation and imported APST scientific modules."""
    paths = [_HERE / name for name in ("training.py", "model.py", "contract.py", "bootstrap.py", "protocol.py", "data.py")]
    return {str(p.relative_to(_PROJECT)): sha256(p) for p in paths if p.is_file()}


def _runtime_hashes() -> dict[str, str]:
    paths = set(_HERE / name for name in ("bootstrap.py", "contract.py", "model.py", "training.py", "protocol.py", "data.py"))
    for module in list(sys.modules.values()):
        path = Path(getattr(module, "__file__", "") or "")
        if path.is_file() and path.suffix == ".py" and _PROJECT in path.resolve().parents:
            paths.add(path.resolve())
    return {str(path.relative_to(_PROJECT)): sha256(path) for path in sorted(paths) if path.is_file()}
...
def _validate_config(config: Mapping[str, Any], *, smoke: bool) -> dict[str, Any]:
    ...
    if c.get("representation") not in {"sua", "pmua"} or c.get("seed") not in {42, 43, 44}:
    ...
    if not smoke and (c.get("source_ids") is not None or c.get("dev_ids") is not None):
        raise ValueError("formal F0 FiLM runs require complete source18/dev6 rosters")
    return c
...
def run_training(config: dict, dest: Path, *, encoder_path: Path | None = None, device: str = "cpu",
                 smoke_updates: int | None = None) -> dict:
    ...
    dev = load_records(Path(c["cache"]), c["representation"], "dev") if not smoke and c["stage"] == "train" else []
