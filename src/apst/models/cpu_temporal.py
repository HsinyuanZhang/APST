...
class CpuRiftTemporalRuntime:
    ...
    def __init__(
        ...
        self.temporal, self.batch_size, self.device, self.dtype = (
            temporal,
            batch_size,
            torch.device(device),
            dtype,
        ...
        self.keys = [
            torch.zeros(batch_size, window - 1, h, dh, device=self.device, dtype=dtype)
        ...
        self._scratch_k = [
            torch.zeros(batch_size, window, h, dh, device=self.device, dtype=dtype)
            for window in c.windows
        ]
        ...
        self._ages = [
            torch.arange(window - 1, -1, -1, device=self.device, dtype=dtype)
            for window in c.windows
        ]
        self._positions = [
            torch.arange(window - 1, device=self.device) for window in c.windows
        ]
    ...
    def step(self, z: Tensor, valid_mask: Tensor | None = None) -> Tensor:
        ...
        if (
            z.shape != (self.batch_size, c.width)
        ...
        for layer, (block, window) in enumerate(zip(self.temporal.blocks, c.windows)):
            qkv = block.qkv(block.norm1(h)).view(
                self.batch_size, 3, c.heads, c.head_dim
            )
            q, k, v = qkv.unbind(1)
            cap = window - 1
            sk, sv, sm = (
            ...
            sk[:, :cap].copy_(self.keys[layer])
            sv[:, :cap].copy_(self.values[layer])
            sm[:, :cap] = self._positions[layer].unsqueeze(0) >= (
            ...
            logits -= self.temporal.recency_slopes.to(dtype=self.dtype).view(
                1, c.heads, 1
            ) * self._ages[layer].view(1, 1, -1)
            logits.masked_fill_(~sm[:, None], float("-inf"))
            logits = torch.where(
                valid_mask[:, None, None], logits, torch.zeros_like(logits)
            )
            attended = torch.einsum(
                "bhk,bkhd->bhd", torch.softmax(logits, -1), sv
            ).reshape(self.batch_size, c.width)
            residual = h + block.out(attended)
            h = residual + block.ffn(block.norm2(residual))
            if cap:
                self.keys[layer].copy_(
                    torch.where(
                        valid_mask[:, None, None, None], sk[:, 1:], self.keys[layer]
                    )
                )
                self.values[layer].copy_(
                    torch.where(
                        valid_mask[:, None, None, None], sv[:, 1:], self.values[layer]
                    )
                )
