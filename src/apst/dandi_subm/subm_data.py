...
def load_pair(session_id: str, *, raw_root: str | Path | None = None,
    ...
    raw_root = protocol.raw_root(raw_root)
...
def load_cached_session(path: Path, final_access: Any = None) -> SessionData:
    with np.load(Path(path), allow_pickle=False) as archive:
        ...
        if split == "final":
            if final_access is None: raise PermissionError("final cache access requires a validated capability")
            final_access.authorize(session_id)
