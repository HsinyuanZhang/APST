...
class CpuLearnableRecencyRuntime:
    ...
    def __init__(
        ...
        self.keys = [
            torch.zeros(
                batch_size, window - 1, heads, head_dim, device=self.device, dtype=dtype
            )
        ...
        self._scratch_k = [
            torch.zeros(
                batch_size, window, heads, head_dim, device=self.device, dtype=dtype
            )
            for window in config.windows
        ]
        self._scratch_v = [torch.zeros_like(value) for value in self._scratch_k]
        self._scratch_a = [
            torch.zeros(batch_size, window, heads, device=self.device, dtype=dtype)
            for window in config.windows
        ]
        self._scratch_valid = [
            torch.zeros(batch_size, window, device=self.device, dtype=torch.bool)
            for window in config.windows
        ]
        self._positions = [
            torch.arange(window - 1, device=self.device) for window in config.windows
        ]
    ...
    def step(self, z: Tensor, valid_mask: Tensor | None = None) -> Tensor:
        ...
        if (
            z.shape != (self.batch_size, config.width)
            or z.device != self.device
            or z.dtype != self.dtype
        ):
            raise ValueError(
                "z must be FP32 [fixed batch_size, configured width] on runtime device"
            )
        ...
        if (
            valid_mask.shape != (self.batch_size,)
            or valid_mask.dtype != torch.bool
            or valid_mask.device != self.device
        ):
            raise ValueError(
                "valid_mask must be bool [fixed batch_size] on runtime device"
            )
        hidden = z
        tokens = z.unsqueeze(1)
        for layer, (block, window) in enumerate(
            zip(self.temporal.blocks, config.windows)
        ):
            increments, gates = self.temporal._increments_and_gates(tokens, layer)
            a_now = increments[:, 0]
            g_now = gates[:, 0]
            qkv = block.qkv(block.norm1(hidden)).view(
                self.batch_size, 3, config.heads, config.head_dim
            )
            q, k, v = qkv.unbind(1)
            cap = window - 1
            ...
            scratch_valid[:, :cap] = self._positions[layer].unsqueeze(0) >= (
                cap - self.lengths[layer]
            ).unsqueeze(1)
            scratch_k[:, cap].copy_(k)
            scratch_v[:, cap].copy_(v)
            scratch_a[:, cap].copy_(a_now)
            scratch_valid[:, cap] = valid_mask
            logits = torch.einsum("bhd,bkhd->bhk", q, scratch_k) * (
                config.head_dim**-0.5
            )
            ages = torch.arange(
                window - 1, -1, -1, device=self.device, dtype=self.dtype
            )
            increment_sums = self.temporal._age_plus_delta_sums(
                scratch_a.permute(0, 2, 1), ages
            )
            ...
            if cap:
                ...
                self.increments[layer].copy_(
                    torch.where(
                        ...
                        scratch_a[:, 1:],
                        self.increments[layer],
                    )
                )
                self.lengths[layer].copy_(
                    torch.where(
                        valid_mask,
                        (self.lengths[layer] + 1).clamp_max(cap),
                        self.lengths[layer],
                    )
                )
    ...
    def reorder(self, rows: Sequence[int] | Tensor) -> None:
        idx = torch.as_tensor(rows, device=self.device, dtype=torch.long)
        if (
            idx.shape != (self.batch_size,)
            or (idx < 0).any()
            or (idx >= self.batch_size).any()
        ):
...
def exported_cpu_runtime(
    ...
    exported.load_state_dict(
        {
            name: value
            for name, value in temporal.state_dict().items()
            if name in exported.state_dict()
        },
        strict=True,
    )
