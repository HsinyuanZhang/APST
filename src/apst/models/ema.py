...
class DecoderEMA:
    ...
    def __init__(self, module: nn.Module, decay: float = plan.EMA_DECAY) -> None:
        plan.require(
            0.0 < float(decay) < 1.0, f"EMA decay must be in (0, 1), got {decay}"
        )
    ...
    def update_after_step(self, module: nn.Module) -> None:
        ...
        plan.require(
            set(named) == set(self.shadow),
            "trainable parameter set changed since EMA init",
        )
        with torch.no_grad():
            if self.n_updates == 0:
    ...
    def load_state_dict(self, state: dict[str, Any]) -> None:
        plan.require(
            "decay" in state and "n_updates" in state and "shadow" in state,
            "EMA state missing keys",
        )
        plan.require(float(state["decay"]) == self.decay, "EMA decay mismatch on load")
        shadow = dict(state["shadow"])
        plan.require(
            set(shadow) == set(self.shadow), "EMA shadow keys mismatch on load"
        )
