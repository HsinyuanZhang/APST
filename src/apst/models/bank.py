"""Frozen per-session calibration bank."""
...
# Defaults used by synthetic test fixtures.
...
class TaskBank:
    ...
    def __post_init__(self) -> None:
        ...
        plan.require(
            e0.ndim == 2 and n_units >= 1, f"E0 must be [N, d_e], got {self.E0.shape}"
        )
        plan.require(
        ...
        plan.require(
            t_store.ndim == 2
            and t_store.shape[0] == x_store.shape[0]
            and t_store.shape[1] >= 1,
        ...
        plan.require(
            ...
            "required metadata: shape / trial_count / estimator / array_sha256 / budget)",
        )
...
def make_synthetic_bank(
    ...
    sets a temporary identity width or prefix for fixture construction.
    ...
    3 (h1) and lands in ``calibration_meta['budget']``; an explicit value mimics a CAL-1 multi-budget bank.
    """
    geometry = plan.task_geometry(task)
    if e0_dim is None:
        e0_dim = plan.resolved_e0_dim(geometry)
    plan.require(
        isinstance(e0_dim, int) and e0_dim >= 1,
        "e0_dim override must be a positive int",
    )
    if budget is None:
        budget = DEFAULT_SYNTHETIC_BUDGET[task]
    plan.require(
        isinstance(budget, int) and budget >= 1, "budget must be a positive int"
    )
    ...
    readout = (rng.standard_normal((out_dim, units)) / np.sqrt(units)).astype(
        np.float32
    )
